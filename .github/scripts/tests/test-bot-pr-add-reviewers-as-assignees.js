...
async function testTeamIgnored(handler) {
  ...
  assert.strictEqual(call.owner, 'hiero-ledger');
  assert.strictEqual(call.repo, 'hiero-sdk-python');
  assert.strictEqual(call.issue_number, 124);
...
async function testPartialDeduplication(handler) {
  ...
  assert.strictEqual(call.owner, 'hiero-ledger');
  assert.strictEqual(call.repo, 'hiero-sdk-python');
  assert.strictEqual(call.issue_number, 131);
...
async function test403Graceful(handler) {
    ...
    rest: {
      ...
      issues: {
        addAssignees: async (params) => {
          ...
          // ✅ Fix: Use proper Error object with status property (matches Octokit)
  ...
  // ✅ Fix: Complete payload like other tests to ensure handler reaches addAssignees
  ...
  // ✅ Expect success (not rejection) - 403 should be caught gracefully
...
async function test500Rethrow(handler) {
  ...
  const ctx = createMockContext({
    pull_request: {
      number: 133,
      requested_reviewers: [{ login: 'x' }],  // Complete payload
      requested_teams: [],
      assignees: []
    }
  });

  // Expect handler to REJECT (rethrow 500 error)
  await assert.rejects(
    () => handler({ github: createErrorMock(500), context: ctx }),
    (err) => {
      // Match Octokit error format
